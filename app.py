...
def upload_file():
    if 'pic' in request.files:
        ...
        # Faites quelque chose avec le fichier, par exemple, le sauvegarder sur le serveur
        db.session.add(Img(img=file.read(), name=file.filename, mimetype=file.content_type))
        db.session.commit()
        #return 'Fichier téléchargé avec succès.'
        return redirect(url_for('get_image', id=1))
    else:
